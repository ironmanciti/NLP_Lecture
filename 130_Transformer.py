...
TRAIN_SIZE = 52000
train_hf = raw_ds["train"].shuffle(seed=42).select(range(TRAIN_SIZE))
val_hf   = raw_ds["validation"]  # 2,000개, 그대로 사용
...
def hf_to_tf_dataset(hf_split):
    pt = [ex["translation"]["pt"] for ex in hf_split]
    en = [ex["translation"]["en"] for ex in hf_split]
    return tf.data.Dataset.from_tensor_slices((pt, en))

train_examples = hf_to_tf_dataset(train_hf)
val_examples   = hf_to_tf_dataset(val_hf)

print(f"\n훈련 예제 수: {len(train_examples):,}")
print(f"검증 예제 수: {len(val_examples):,}")
